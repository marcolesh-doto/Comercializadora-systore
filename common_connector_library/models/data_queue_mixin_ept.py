...
class DataQueueMixinEpt(models.AbstractModel):
    ...
    def delete_data_queue_schedule_activity_ept(self, tbl_name, is_delete_queue=False):
        ...
        if is_delete_queue:
            ...
            self._cr.execute("DELETE FROM mail_message WHERE model = %s", (model_name,))
        else:
            self._cr.execute(
            ...
            if records:
                if len(records) == 1:
                    ...
                    self._cr.execute(f"delete from mail_message where res_id={records[0]} and model = %s",
                                     (model_name,))
                else:
                    delete_activity = f"delete from mail_activity where res_id in {records} and res_model_id={model.id}"
                    self._cr.execute(f"delete from mail_message where res_id in {records} and model = %s",
                                     (model_name,))
